# License & Usage

Documentation license and usage terms.

## Documentation License

This manual is © 2026 Zobaer Al, Mineno Laboratory, Shizuoka University.

**Usage Rights:**
- ✅ Read and reference
- ✅ Use for research
- ✅ Share within lab
- ✅ Cite in publications

**Restrictions:**
- ❌ Commercial use without permission
- ❌ Redistribution without attribution
- ❌ Modification without notice

## Software Licenses

### 3D Gaussian Splatting
- Original: Inria (License unclear, check repository)
- URL: https://github.com/graphdeco-inria/gaussian-splatting

### COLMAP
- License: BSD 3-Clause
- URL: https://github.com/colmap/colmap

### CUDA
- License: NVIDIA EULA
- URL: https://developer.nvidia.com/cuda-toolkit

## Citation

If using this manual or methodology:

```bibtex
@misc{al2026_3dgs_manual,
  author = {Al, Zobaer},
  title = {3D Gaussian Splatting Pipeline Manual},
  year = {2026},
  publisher = {Mineno Laboratory, Shizuoka University},
  url = {https://alzobaer.github.io/3dgs-pipeline-manual/}
}
```

## Contact

**Zobaer Al**  
Mineno Laboratory  
Shizuoka University  
Email: al.zobaer.24@shizuoka.ac.jp

For questions, issues, or permissions.
